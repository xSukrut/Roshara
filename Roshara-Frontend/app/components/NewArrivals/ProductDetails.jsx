...
function imagesFromProduct(product) {
  ...
  const pick = (img) => {
    if (!img) return null;
    const raw = typeof img === "string" ? img : (img.url || img.src || img.path || img.location || img.file);
    if (!raw) return null;
    if (raw.startsWith("http")) return raw;
    if (raw.startsWith("/uploads")) return `${base}${raw}`;
    return raw;
  };
  const arr = (product?.images?.length ? product.images : [product?.image]).map(pick).filter(Boolean);
      ...
      <div className="relative bg-white w-full max-w-5xl rounded-2xl overflow-hidden shadow-2xl">
        ...
        <div className="grid md:grid-cols-2 gap-6 p-6">
          ...
          <div>
            ...
            <div className="mt-4 text-3xl font-semibold">₹{Number(product.price).toLocaleString("en-IN")}</div>
            ...
            <div className="mt-6 flex gap-3">
              <button className="bg-black text-white px-6 py-3 rounded hover:bg-gray-800" onClick={addToCart}>
                Add to Cart
              </button>
            </div>
